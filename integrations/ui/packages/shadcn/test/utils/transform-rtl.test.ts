...
describe("applyRtlMapping", () => {
  ...
  test("handles named group selectors with data attributes", () => {
    expect(applyRtlMapping("sm:group-data-[size=default]/alert-dialog-content:text-left")).toBe(
  ...
  test("transforms slide animations inside logical side variants", () => {
    expect(applyRtlMapping("data-[side=inline-start]:slide-in-from-right-2")).toBe(
      "data-[side=inline-start]:slide-in-from-end-2"
    );
    expect(applyRtlMapping("data-[side=inline-start]:slide-out-to-right-2")).toBe(
      "data-[side=inline-start]:slide-out-to-end-2"
    );
  ...
  test("adds rtl: variant for translate-x classes", () => {
    expect(applyRtlMapping("-translate-x-1/2")).toBe("-translate-x-1/2 rtl:translate-x-1/2");
    expect(applyRtlMapping("translate-x-full")).toBe("translate-x-full rtl:-translate-x-full");
    expect(applyRtlMapping("-translate-x-px")).toBe("-translate-x-px rtl:translate-x-px");
  });
  ...
  test("handles space-x and divide-x with variant prefixes", () => {
    expect(applyRtlMapping("md:space-x-4")).toBe("md:space-x-4 rtl:md:space-x-reverse");
    expect(applyRtlMapping("hover:divide-x-2")).toBe("hover:divide-x-2 rtl:hover:divide-x-reverse");
  });
  ...
  test("adds rtl: variant for cursor resize classes", () => {
    expect(applyRtlMapping("cursor-w-resize")).toBe("cursor-w-resize rtl:cursor-e-resize");
    expect(applyRtlMapping("cursor-e-resize")).toBe("cursor-e-resize rtl:cursor-w-resize");
  });
  ...
  test("does not add logical side selectors without cn-logical-sides marker", () => {
    ...
    expect(applyRtlMapping("data-[side=left]:top-1")).toBe("data-[side=left]:top-1");
  });
  ...
  test("does not transform positioning classes inside physical side variants", () => {
    ...
    expect(applyRtlMapping("data-[side=left]:-right-1")).toBe("data-[side=left]:-right-1");
    expect(applyRtlMapping("data-[side=right]:-left-1")).toBe("data-[side=right]:-left-1");
    expect(applyRtlMapping("data-[side=left]:right-0")).toBe("data-[side=left]:right-0");
    expect(applyRtlMapping("data-[side=right]:left-0")).toBe("data-[side=right]:left-0");
  });
  ...
  test("still transforms non-positioning classes inside physical side variants", () => {
    ...
    expect(applyRtlMapping("data-[side=left]:ml-2")).toBe("data-[side=left]:ms-2");
    expect(applyRtlMapping("data-[side=right]:pl-4")).toBe("data-[side=right]:ps-4");
    expect(applyRtlMapping("data-[side=left]:text-left")).toBe("data-[side=left]:text-start");
  });
