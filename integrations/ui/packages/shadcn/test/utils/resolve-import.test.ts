...
test("resolve import with base url", async () => {
  ...
  expect(await resolveImport("@/lib/utils", config)).toEqual(path.resolve(cwd, "lib/utils"));
  expect(await resolveImport("foo/bar", config)).toEqual(path.resolve(cwd, "foo/bar"));
});
