...
async function buildRegistry(opts: z.infer<typeof buildOptionsSchema>) {
  try {
    ...
    if (!result.success) {
      logger.error(
        `Invalid registry file found at ${highlighter.info(resolvePaths.registryFile)}.`
      );
      logger.break();
      process.exit(1);
    }
    ...
    const resolvedRegistry = await resolveRegistryItems(result.data, config, projectInfo);
    ...
    if (options.verbose) {
      spinner(
        `The registry has ${highlighter.info(resolvedRegistry.items.length.toString())} items:`
      ).succeed();
...
async function resolveRegistryItems(
  ...
  for (const item of registry.items) {
    ...
    for (const file of item.files) {
      const results = await recursivelyResolveFileImports(file.path, config, projectInfo);
