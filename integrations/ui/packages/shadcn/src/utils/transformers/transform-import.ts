...
export const transformImport: Transformer = async ({ sourceFile, config, isRemote }) => {
  const utilsAlias = config.aliases?.utils;
  const workspaceAlias =
    typeof utilsAlias === "string" && utilsAlias.includes("/") ? utilsAlias.split("/")[0] : "@";
  ...
  for (const specifier of sourceFile.getImportStringLiterals()) {
    const updated = updateImportAliases(specifier.getLiteralValue(), config, isRemote);
    specifier.setLiteralValue(updated);

    // Replace `import { cn } from "@/lib/utils"`
    if (utilsImport === updated || updated === "@/lib/utils") {
      const importDeclaration = specifier.getFirstAncestorByKind(SyntaxKind.ImportDeclaration);
...
function updateImportAliases(moduleSpecifier: string, config: Config, isRemote: boolean = false) {
  // Not a local import.
