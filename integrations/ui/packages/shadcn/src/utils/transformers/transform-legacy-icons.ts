...
export const transformLegacyIcons: Transformer = async ({ sourceFile, config }) => {
  ...
  if (targetedIcons.length > 0) {
    const iconImportDeclaration = sourceFile.addImportDeclaration({
      moduleSpecifier:
        LEGACY_ICON_LIBRARIES[targetLibrary as keyof typeof LEGACY_ICON_LIBRARIES]?.import,
    ...
    if (!_useSemicolon(sourceFile)) {
      iconImportDeclaration.replaceWithText(iconImportDeclaration.getText().replace(";", ""));
    }
  }
