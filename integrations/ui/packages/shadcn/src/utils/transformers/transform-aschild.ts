...
const ELEMENTS_REQUIRING_NATIVE_BUTTON_FALSE = ["a", "span", "div", "Link", "label", "Label"];
...
export const transformAsChild: Transformer = async ({ sourceFile, config }) => {
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    const leafElements = asChildElements.filter((el) => {
      ...
      return !descendants.some((d) => d.getOpeningElement().getAttribute("asChild"));
    });
    ...
    for (const jsxElement of leafElements) {
      ...
      if (childElement.getKind() === SyntaxKind.JsxSelfClosingElement) {
        const selfClosing = childElement.asKindOrThrow(SyntaxKind.JsxSelfClosingElement);
      ...
      const needsNativeButton =
        parentTagName === "Button" && ELEMENTS_REQUIRING_NATIVE_BUTTON_FALSE.includes(childTagName);
