...
import { Node, NoSubstitutionTemplateLiteral, StringLiteral, SyntaxKind } from "ts-morph";
...
async function getSupportedFontMarkers(tailwindCssPath?: string, extraMarkers: string[] = []) {
...
function rewriteFontMarkers(className: string, supportedMarkers: Set<string>): string {
  let next = className;
...
function processStringLiteral(node: StringLikeLiteral, supportedMarkers: Set<string>) {
...
function processStringLiterals(node: Node, supportedMarkers: Set<string>) {
  ...
  for (const templateLit of node.getDescendantsOfKind(SyntaxKind.NoSubstitutionTemplateLiteral)) {
    processStringLiteral(templateLit, supportedMarkers);
  }
}

export const transformFont: Transformer = async ({ sourceFile, config, supportedFontMarkers }) => {
