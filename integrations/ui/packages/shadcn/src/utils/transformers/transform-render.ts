...
export const transformRender: Transformer = async ({ sourceFile, config }) => {
    ...
    const otherAttrs = openingElement
      ...
      .filter((attr) => {
        if (attr.getKind() === SyntaxKind.JsxAttribute) {
          const attrName = attr.asKindOrThrow(SyntaxKind.JsxAttribute).getNameNode().getText();
  ...
  for (const info of transformations.reverse()) {
    ...
    const newFullText =
      fullText.substring(0, info.elementStart) + info.newText + fullText.substring(info.elementEnd);
