...
import { CallExpression, Project, ScriptKind, SyntaxKind, VariableDeclarationKind } from "ts-morph";
import z from "zod";
...
const ROOT_FONT_VARIABLES = new Set(["--font-sans", "--font-serif", "--font-mono"]);
...
export async function massageTreeForFonts(
  ...
  const isNext =
    projectInfo.framework.name === "next-app" || projectInfo.framework.name === "next-pages";
  ...
  if (tree.fonts.length > 0) {
    ...
    for (const [selector, classes] of Array.from(groups.entries())) {
      ...
      const existingApplyKey = Object.keys(tree.css["@layer base"][selector]).find((key) =>
...
async function updateNextFonts(
  ...
  const updatedContent = await transformLayoutFonts(layoutContent, fonts, config);
...
export async function transformLayoutFonts(
  ...
  for (const font of googleFonts) {
    ...
    if (existingImport) {
      ...
      hasExistingImport = namedImports.some((imp) => imp.getName() === importName);
    ...
    const existingVarDecl = findFontVariableDeclaration(sourceFile, font.font.variable);
  ...
  const filteredUtilityClasses = fontUtilityClasses.filter((cls) => !fontFamilyClasses.has(cls));
  ...
  if (fontVariableNames.length > 0) {
    updateHtmlClassName(sourceFile, fontVariableNames, filteredUtilityClasses, config);
  }
...
function findInsertPosition(sourceFile: ReturnType<Project["createSourceFile"]>) {
...
function updateHtmlClassName(
  ...
  for (const element of jsxElements) {
    ...
    const newVarExpressions = fontVariableNames.map((name) => `${name}.variable`);
    ...
    if (initializer.getKind() === SyntaxKind.StringLiteral) {
      ...
      ensureCnImport(sourceFile, config);
      jsxAttr.setInitializer(`{cn("${currentValue}", ${allNewArgs.join(", ")})}`);
      ...
      if (exprText.startsWith("cn(")) {
        ...
        const hasAllFontVars = newVarExpressions.every((v) => exprText.includes(v));
        ...
        const shouldPreserveExisting =
          existingName.toLowerCase().includes("heading") || fontUtilityClasses.length === 0;
        ...
        jsxExpr.replaceWithText(`{cn(${[...cleanedCnArgs, ...allNewArgs].join(", ")})}`);
      } else {
...
function ensureCnImport(sourceFile: ReturnType<Project["createSourceFile"]>, config: Config) {
  const existingImport = sourceFile.getImportDeclaration((decl) => {
...
function removeFontVariablesFromCn(cnExpr: string, variablesToRemove: string[]) {
