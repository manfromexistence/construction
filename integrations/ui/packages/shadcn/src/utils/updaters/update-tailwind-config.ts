  ...
  const configObject = sourceFile
    ...
    .find((node) =>
      node
        ...
        .some(
          (property) =>
            property.isKind(SyntaxKind.PropertyAssignment) && property.getName() === "content"
        )
    );
  ...
  for (let i = 0; i < properties.length; i++) {
    ...
    if (prop.isKind(SyntaxKind.PropertyAssignment)) {
      ...
      if (initializer && initializer.isKind(SyntaxKind.StringLiteral)) {
        ...
        unsetSpreadElements(initializer.asKindOrThrow(SyntaxKind.ArrayLiteralExpression));
      }
    }
  }
}
...
export function unsetSpreadElements(arr: ArrayLiteralExpression) {
  ...
  for (let j = 0; j < elements.length; j++) {
    ...
    if (element.isKind(SyntaxKind.ObjectLiteralExpression)) {
      ...
      // Recursive check on nested arrays
      unsetSpreadElements(element.asKindOrThrow(SyntaxKind.ArrayLiteralExpression));
