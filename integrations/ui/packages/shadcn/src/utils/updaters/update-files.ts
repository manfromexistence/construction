...
export async function updateFiles(
  ...
  filesUpdated = Array.from(new Set(filesUpdated.filter((file) => !filesCreated.includes(file))));
...
function resolveFileTargetDirectory(file: z.infer<typeof registryItemFileSchema>, config: Config) {
...
export function resolveNestedFilePath(filePath: string, targetDir: string): string {
...
export function resolvePageTarget(target: string, framework?: ProjectInfo["framework"]["name"]) {
  if (!framework) {
    return "";
  }
...
async function resolveImports(filePaths: string[], config: Config) {
  ...
  for (const filepath of filePaths) {
    ...
    const sourceFile = project.createSourceFile(path.join(dir, basename(resolvedPath)), content, {
    ...
    for (const importDeclaration of importDeclarations) {
      ...
      const probableImportFilePath = await resolveImport(moduleSpecifier, tsConfig);
...
export function toAliasedImport(
  ...
  const matches = Object.entries(config.resolvedPaths)
    .filter(([, root]) => root && abs.startsWith(path.normalize(root + path.sep)))
...
function _isNext16Middleware(filePath: string, projectInfo: ProjectInfo | null, config: Config) {
