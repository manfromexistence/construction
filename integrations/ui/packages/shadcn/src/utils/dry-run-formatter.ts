...
function formatSummaryOutput(result: DryRunResult, componentNames: string[]) {
  ...
  if (overwriteCount > 0) {
    lines.push(
      yellow(`⚠ ${overwriteCount} ${overwriteCount === 1 ? "file" : "files"} will be overwritten.`)
    );
    lines.push(dim("│"));
  }
...
function formatFileDiff(file: DryRunFile, lines: string[]) {
  lines.push(`${dim("├")} ${bold(file.path)} ${dim("(")}${colorAction(file.action)}${dim(")")}`);
  ...
  if (file.action === "skip") {
    ...
    const diffLines = computeUnifiedDiff(file.existingContent!, file.content, file.path);
    pushContentBox(lines, diffLines);
  }
...
function formatFilesSection(result: DryRunResult, lines: string[]) {
  ...
  lines.push(`${dim("├")} ${bold("Files")} ${dim(`(${result.files.length})`)}${summary}`);
  ...
  for (const file of result.files) {
    ...
    lines.push(`${dim("│")} ${colorFn(glyph)} ${pathStr}${padding}${colorFn(label)}`);
  }
...
function processChangeGroup(
  ...
  for (let j = 0; j < added.length; j++) {
    ...
    if (matchIdx !== -1) {
      ...
      const unmatchedIdx = normalizedCollapsed.findIndex((_, idx) => !usedCollapsed.has(idx));
      if (unmatchedIdx !== -1) {
