...
import { formatMonorepoMessage, getMonorepoTargets, isMonorepoRoot } from "./get-monorepo-info";
...
describe("getMonorepoTargets", () => {
  it("should find targets from pnpm-workspace.yaml", async () => {
    ...
    await fs.writeFile(path.join(webDir, "next.config.mjs"), "export default {}");
  ...
  it("should find multiple targets", async () => {
    ...
    await fs.writeFile(path.join(docsDir, "vite.config.ts"), "export default {}");
  ...
  it("should skip directories without package.json", async () => {
    ...
    await fs.writeFile(path.join(libDir, "next.config.mjs"), "export default {}");
  ...
  it("should detect astro, remix, and svelte configs", async () => {
    ...
    await fs.writeFile(path.join(astroDir, "astro.config.mjs"), "export default {}");
...
describe("formatMonorepoMessage", () => {
  it("should log the monorepo message with targets", () => {
    ...
    expect(allLogCalls.some((msg) => msg.includes("shadcn init -c apps/web"))).toBe(true);
    expect(allLogCalls.some((msg) => msg.includes("shadcn init -c apps/docs"))).toBe(true);
  ...
  it("should use the correct command name", () => {
    ...
    formatMonorepoMessage("add [component]", [{ name: "apps/web", hasConfig: false }]);

    const allLogCalls = logSpy.mock.calls.map((c) => c[0] as string);
    expect(allLogCalls.some((msg) => msg.includes("shadcn add [component] -c apps/web"))).toBe(
      true
    );
