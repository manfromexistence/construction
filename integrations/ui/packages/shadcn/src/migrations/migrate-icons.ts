...
export async function migrateIcons(config: Config) {
  ...
  const libraryChoices = Object.entries(LEGACY_ICON_LIBRARIES).map(([name, iconLibrary]) => ({
  ...
  const sourceLibrary =
    LEGACY_ICON_LIBRARIES[migrateOptions.sourceLibrary as keyof typeof LEGACY_ICON_LIBRARIES];
  const targetLibrary =
    LEGACY_ICON_LIBRARIES[migrateOptions.targetLibrary as keyof typeof LEGACY_ICON_LIBRARIES];
  const { confirm } = await prompts({
    type: "confirm",
    name: "confirm",
    initial: true,
    message: `We will migrate ${highlighter.info(files.length)} files in ${highlighter.info(
