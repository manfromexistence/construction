...
describe("resolveRegistryNamespaces", () => {
  ...
  it("should discover namespaces from direct components", async () => {
    ...
    const namespaces = await resolveRegistryNamespaces(["@foo/button", "@bar/card"], mockConfig);
  ...
  it("should skip built-in registries", async () => {
    ...
    const namespaces = await resolveRegistryNamespaces(["@shadcn/button", "@foo/card"], mockConfig);
  ...
  it("should discover namespaces from registry dependencies", async () => {
    ...
    mockFetchRegistryItems
      ...
      .mockResolvedValueOnce([{ name: "button", type: "registry:ui", files: [], dependencies: [] }])
      .mockResolvedValueOnce([{ name: "modal", type: "registry:ui", files: [], dependencies: [] }]);

    const namespaces = await resolveRegistryNamespaces(["@foo/dialog"], mockConfig);
  ...
  it("should handle circular dependencies", async () => {
    ...
    const namespaces = await resolveRegistryNamespaces(["@foo/comp-a"], mockConfig);
  ...
  it("should handle RegistryNotConfiguredError gracefully", async () => {
    const mockFetchRegistryItems = vi.mocked(resolver.fetchRegistryItems);
    mockFetchRegistryItems.mockRejectedValue(new RegistryNotConfiguredError("@unknown"));

    const namespaces = await resolveRegistryNamespaces(["@unknown/button"], mockConfig);
  ...
  it("should continue processing on other errors", async () => {
    ...
    mockFetchRegistryItems
      ...
      .mockResolvedValueOnce([{ name: "card", type: "registry:ui", files: [], dependencies: [] }]);
  ...
  it("should handle deeply nested dependencies", async () => {
    ...
    const namespaces = await resolveRegistryNamespaces(["@level1/level-1"], mockConfig);
  ...
  it("should return unique namespaces", async () => {
    ...
    mockFetchRegistryItems
      ...
      .mockResolvedValueOnce([{ name: "shared", type: "registry:ui", files: [], dependencies: [] }])
      .mockResolvedValueOnce([
  ...
  it("should handle components without namespace", async () => {
    ...
    const namespaces = await resolveRegistryNamespaces(["button", "@foo/card"], mockConfig);
  ...
  it("should discover namespaces from URL components with registryDependencies", async () => {
    ...
    expect(mockFetchRegistryItems).toHaveBeenCalledWith(["@ai-elements/branch"], mockConfig, {
      useCache: true,
    });
    expect(mockFetchRegistryItems).toHaveBeenCalledWith(["@8bitcn/button"], mockConfig, {
      useCache: true,
    });
  });
});
...
describe("checkNamespaceConfiguration", () => {
  ...
  it("should identify missing namespaces", () => {
    const result = checkNamespaceConfiguration(["@foo", "@unknown", "@missing"], mockConfig);
  ...
  it("should handle config without registries", () => {
    ...
    const result = checkNamespaceConfiguration(["@foo", "@bar"], configWithoutRegistries);
