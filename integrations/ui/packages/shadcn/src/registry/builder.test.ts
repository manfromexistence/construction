...
describe("buildUrlFromRegistryConfig", () => {
  ...
  it("should build URL from string config", () => {
    const url = buildUrlFromRegistryConfig("chat-component", "https://v0.dev/chat/b/{name}/json");
  ...
  it("should build URL with env vars", () => {
    const url = buildUrlFromRegistryConfig("button", "https://api.com/{name}?token=${TEST_TOKEN}");
...
describe("buildHeadersFromRegistryConfig", () => {
  ...
  it("should return empty object for config without headers", () => {
    expect(buildHeadersFromRegistryConfig({ url: "https://api.com/{name}" })).toEqual({});
  });
...
describe("buildUrlAndHeadersForRegistryItem", () => {
  ...
  it("should handle URLs and local files", () => {
    ...
    expect(buildUrlAndHeadersForRegistryItem("https://example.com/button", config)).toBeNull();

    // Local files should return null (not registry items)
    expect(buildUrlAndHeadersForRegistryItem("./local/button", config)).toBeNull();
  });
});
