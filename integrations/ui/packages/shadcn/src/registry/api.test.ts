...
describe("getRegistryItem", () => {
  ...
  it("should throw error for JSON that doesn't match registry schema", async () => {
    ...
    try {
      await expect(getRegistryItems([tempFile])).rejects.toThrow(RegistryParseError);
    } finally {
      // Clean up
      await fs.unlink(tempFile);
      await fs.rmdir(tempDir);
  ...
  it("should include error code in RegistryUnauthorizedError", async () => {
    ...
    try {
      ...
      if (error instanceof RegistryUnauthorizedError) {
        ...
        expect(error.suggestion).toContain("Check your authentication credentials");
  ...
  it("should include error code in RegistryFetchError for 500 errors", async () => {
    ...
    try {
      ...
      if (error instanceof RegistryFetchError) {
        ...
        expect(error.suggestion).toContain("The registry server encountered an error");
  ...
  it("should extract Zod validation issues in RegistryParseError", async () => {
    ...
    try {
      ...
      if (error instanceof RegistryParseError) {
        ...
        expect(error.suggestion).toContain("may be corrupted or have an invalid format");
...
describe("getRegistry", () => {
  ...
  it("should throw RegistryNotConfiguredError when registry is not configured", async () => {
    ...
    await expect(getRegistry("@unknown", { config: mockConfig })).rejects.toThrow(
      RegistryNotConfiguredError
    );
  });
  ...
  it("should throw RegistryParseError on invalid registry data", async () => {
    ...
    await expect(getRegistry("@invalid", { config: mockConfig })).rejects.toThrow(
      RegistryParseError
    );
  });
  ...
  it("should throw RegistryFetchError on network error", async () => {
    ...
    await expect(getRegistry("@error", { config: mockConfig })).rejects.toThrow(RegistryFetchError);
  });
  ...
  it("should throw RegistryNotConfiguredError when registry is not in config", async () => {
    ...
    await expect(getRegistry("@nonexistent", { config: mockConfig })).rejects.toThrow(
      RegistryNotConfiguredError
    );
  });
  ...
  it("should handle 404 error from registry endpoint", async () => {
    ...
    await expect(getRegistry("@notfound", { config: mockConfig })).rejects.toThrow(
      RegistryNotFoundError
    );
  });
  ...
  it("should handle 401 error from registry endpoint", async () => {
    ...
    await expect(getRegistry("@unauthorized", { config: mockConfig })).rejects.toThrow(
      RegistryUnauthorizedError
    );
  });
  ...
  it("should handle 403 error from registry endpoint", async () => {
    ...
    await expect(getRegistry("@forbidden", { config: mockConfig })).rejects.toThrow(
      RegistryForbiddenError
    );
  });
  ...
  it("should handle malformed JSON response", async () => {
    ...
    await expect(getRegistry("@malformed", { config: mockConfig })).rejects.toThrow();
  });
  ...
  it("should throw RegistryInvalidNamespaceError for registry name without @ prefix", async () => {
    ...
    await expect(getRegistry("invalid-name", { config: mockConfig })).rejects.toThrow(
      RegistryInvalidNamespaceError
    );
  });
  ...
  it("should handle malformed URL gracefully", async () => {
    ...
    await expect(getRegistry(badUrl)).rejects.toThrow(RegistryInvalidNamespaceError);
  });
...
describe("getRegistriesConfig", () => {
  ...
  it("should handle complex registry configurations", async () => {
    ...
    try {
      ...
      expect(result.registries?.["@acme"]).toBe("https://acme.com/registry/{name}.json");
      ...
      expect(result.registries?.["@local"]).toBe("file:///local/registry/{name}.json");
    } finally {
      await fs.unlink(configFile);
      await fs.rmdir(tempDir);
    }
  ...
  describe("caching behavior", () => {
    it("should use cache by default", async () => {
      ...
      try {
        ...
        expect(result1.registries?.["@cached"]).toBe("https://cached.com/{name}.json");
        ...
        expect(result2.registries?.["@cached"]).toBe("https://cached.com/{name}.json");
      } finally {
        await fs.unlink(configFile);
        await fs.rmdir(tempDir);
      }
    ...
    it("should use cache when explicitly enabled", async () => {
      ...
      try {
        ...
        expect(result1.registries?.["@test"]).toBe("https://test.com/{name}.json");
        ...
        expect(result2.registries?.["@test"]).toBe("https://test.com/{name}.json");
      } finally {
        await fs.unlink(configFile);
        await fs.rmdir(tempDir);
      }
    ...
    it("should bypass cache when useCache is false", async () => {
      ...
      try {
        ...
        expect(result1.registries?.["@nocache"]).toBe("https://nocache.com/{name}.json");
        ...
        expect(result2.registries?.["@nocache"]).toBe("https://fresh.com/{name}.json");
      } finally {
        await fs.unlink(configFile);
        await fs.rmdir(tempDir);
      }
    ...
    it("should clear cache for subsequent calls when useCache is false", async () => {
      ...
      try {
        // First call with cache enabled
        const result1 = await getRegistriesConfig(tempDir, { useCache: true });
        expect(result1.registries?.["@step1"]).toBe("https://step1.com/{name}.json");
        ...
        const result2 = await getRegistriesConfig(tempDir, { useCache: false });
        expect(result2.registries?.["@step2"]).toBe("https://step2.com/{name}.json");
        ...
        expect(result3.registries?.["@step3"]).toBe("https://step3.com/{name}.json");
        ...
        expect(result4.registries?.["@step3"]).toBe("https://step3.com/{name}.json");
      } finally {
        await fs.unlink(configFile);
        await fs.rmdir(tempDir);
      }
    ...
    it("should handle invalid config with cache options", async () => {
      ...
      try {
        ...
        await expect(getRegistriesConfig(tempDir, { useCache: true })).rejects.toThrow(
          ConfigParseError
        );

        await expect(getRegistriesConfig(tempDir, { useCache: false })).rejects.toThrow(
          ConfigParseError
        );
      } finally {
        await fs.unlink(configFile);
  ...
  describe("getRegistries", () => {
    ...
    it("should handle invalid JSON response", async () => {
      ...
      await expect(getRegistries({ useCache: false })).rejects.toThrow(RegistriesIndexParseError);
    });
  });
