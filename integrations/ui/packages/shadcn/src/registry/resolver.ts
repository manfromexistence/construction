...
import { z } from "zod";
import { getRegistryBaseColor, getShadcnRegistryIndex } from "@/src/registry/api";
import { buildUrlAndHeadersForRegistryItem, resolveRegistryUrl } from "@/src/registry/builder";
import { setRegistryHeaders } from "@/src/registry/context";
import { RegistryNotConfiguredError, RegistryParseError } from "@/src/registry/errors";
import { fetchRegistry, fetchRegistryLocal } from "@/src/registry/fetcher";
import { parseRegistryAndItemFromString } from "@/src/registry/parser";
import { deduplicateFilesByTarget, isLocalFile, isUrl } from "@/src/registry/utils";
import {
  RegistryFontItem,
...
export async function resolveRegistryTree(
  ...
  const sourceMap = new Map<z.infer<typeof registryItemWithSourceSchema>, string>();
...
function topologicalSortRegistryItems(
  ...
  const itemMap = new Map<string, z.infer<typeof registryItemWithSourceSchema>>();
  const hashToItem = new Map<string, z.infer<typeof registryItemWithSourceSchema>>();
