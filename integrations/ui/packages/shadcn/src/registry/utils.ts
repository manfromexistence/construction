...
import { z } from "zod";
import { configSchema, registryItemFileSchema, registryItemSchema } from "@/src/schema";
...
export function getDependencyFromModuleSpecifier(moduleSpecifier: string): string | null {
...
export async function recursivelyResolveFileImports(
  ...
  const relativeRegistryFilePath = path.relative(config.resolvedPaths.cwd, resolvedFilePath);
  ...
  for (const importStatement of importStatements) {
    ...
    const isAliasImport = moduleSpecifier.startsWith(`${projectInfo.aliasPrefix}/`);
    ...
    if (isRelativeImport) {
      probableImportFilePath = path.resolve(path.dirname(resolvedFilePath), moduleSpecifier);
    }
...
function determineFileType(moduleSpecifier: string): z.infer<typeof registryItemSchema>["type"] {
...
export function isUniversalRegistryItem(
  registryItem: Pick<z.infer<typeof registryItemSchema>, "files" | "type"> | null | undefined
): boolean {
  if (!registryItem) {
    return false;
  }
