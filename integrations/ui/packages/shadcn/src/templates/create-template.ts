...
const GITHUB_REPO_URL = process.env.SHADCN_GITHUB_URL ?? "https://github.com/shadcn-ui/ui.git";
...
async function adaptWorkspaceConfig(projectPath: string, packageManager: string) {
...
function defaultScaffold({ title, templateDir }: { title: string; templateDir: string }) {
  return async ({ projectPath, packageManager }: TemplateOptions) => {
    ...
    try {
      ...
      if (localTemplateDir) {
        ...
        const extractedPath = path.resolve(templatePath, "templates", templateDir);
      ...
      createSpinner?.fail(`Something went wrong creating a new ${title} project.`);
      handleError(error);
    }
  };
}
