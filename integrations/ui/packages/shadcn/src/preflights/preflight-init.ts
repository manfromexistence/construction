...
export async function preFlightInit(options: z.infer<typeof initOptionsSchema>) {
  ...
  const tailwind = options.existingConfig?.tailwind as Record<string, unknown> | undefined;
  ...
  if (projectInfo.tailwindVersion === "v4") {
    tailwindSpinnerMessage = `Validating Tailwind CSS. Found ${highlighter.info("v4")}.`;
  }
