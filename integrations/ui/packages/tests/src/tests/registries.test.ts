...
import { createFixtureTestDirectory, cssHasProperties, npxShadcn } from "../utils/helpers";
...
describe("registries", () => {
  it("should add from registry using url", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "components/foo.tsx"))).toBe(true);
  });
  ...
  it("should add multiple items from registry using urls", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "components/foo.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/bar.tsx"))).toBe(true);
  });
  ...
  it("should add multiple items from multiple registries using urls", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "components/foo.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "example/one.txt"))).toBe(true);
  });

  it("should add item using name and from registry url", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init");
    await npxShadcn(fixturePath, ["add", "alert-dialog", "http://localhost:4444/r/foo"]);

    expect(await fs.pathExists(path.join(fixturePath, "components/ui/alert-dialog.tsx"))).toBe(
      true
    );
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/foo.tsx"))).toBe(true);
  });
  ...
  it("should add item from name, local path and registry url", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "path/to/foo.txt"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/alert-dialog.tsx"))).toBe(
      true
    );
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx"))).toBe(true);
  ...
  it("should show an error when adding from a non-existent registry", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init");
    const output = await npxShadcn(fixturePath, ["add", "@non-existent/component"]);
  ...
  it("should add registry dependencies when adding from namespaced registry", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "components/baz.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/bar.tsx"))).toBe(true);
  });
  ...
  it("should add nested registry dependencies when adding from namespaced registry", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "components/qux.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/baz.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/bar.tsx"))).toBe(true);
  });
  ...
  it("should show an error when adding url with namespaced dependency", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init");
    const output = await npxShadcn(fixturePath, ["add", "http://localhost:4444/r/baz"]);
  ...
  it("should show an error when adding url with unconfigured registry", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init");
    const output = await npxShadcn(fixturePath, ["add", "http://localhost:5555/registry/two"]);
  ...
  it("should show an error when adding multiple namespaced items with unconfigured registry", async () => {
    ...
    const output = await npxShadcn(fixturePath, ["add", "@two/one", "@foobar/foo"]);
  ...
  it("should add item when authentication is configured and nested dependencies are authorized", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "app/quux/page.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/two.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/example.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/qux.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/bar.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/baz.tsx"))).toBe(true);
  ...
  describe("fetchFromRegistry improvements", () => {
    ...
    it("should handle circular dependencies gracefully", async () => {
            ...
            files: [
              {
                ...
                content: "export function CompA() { return <div>Component A</div> }",
                type: "registry:component",
              },
            ],
          },
            ...
            files: [
              {
                ...
                content: "export function CompB() { return <div>Component B</div> }",
                type: "registry:component",
              },
            ],
          },
      ...
      expect(await fs.pathExists(path.join(fixturePath, "components/comp-a.tsx"))).toBe(true);
      expect(await fs.pathExists(path.join(fixturePath, "components/comp-b.tsx"))).toBe(true);
    ...
    it("should preserve header context across dependency resolution", async () => {
            ...
            files: [
              {
                ...
                content: "export function Parent() { return <div>Parent</div> }",
                type: "registry:component",
              },
            ],
          },
            ...
            files: [
              {
                ...
                content: "export function Child1() { return <div>Child 1</div> }",
                type: "registry:component",
              },
            ],
          },
            ...
            files: [
              {
                ...
                content: "export function Child2() { return <div>Child 2</div> }",
                type: "registry:component",
              },
            ],
          },
      ...
      expect(await fs.pathExists(path.join(fixturePath, "components/parent.tsx"))).toBe(true);
      expect(await fs.pathExists(path.join(fixturePath, "components/child1.tsx"))).toBe(true);
      expect(await fs.pathExists(path.join(fixturePath, "components/child2.tsx"))).toBe(true);
  ...
  describe("built-in registries", () => {
    it("should error when trying to override @shadcn in config", async () => {
      ...
      expect(combinedOutput.toLowerCase()).toMatch(/built-in registry|invalid configuration/);
    });
    ...
    it("should work with @shadcn namespace without configuration", async () => {
      ...
      if (!(await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx")))) {
        console.log("Test failed. Command output:", output.stdout);
        console.log("Command stderr:", output.stderr);
      }
  ...
  it("should add registry:item with no framework", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "path/to/foo.txt"))).toBe(true);
    expect(await fs.readFile(path.join(fixturePath, "path/to/foo.txt"), "utf-8")).toBe("Foo Bar");
  });
  ...
  it("should add registry:theme", async () => {
    ...
    const output = await npxShadcn(fixturePath, ["add", "@one/foo-theme", "--yes"]);

    const globalCssContent = await fs.readFile(path.join(fixturePath, "app/globals.css"), "utf-8");
  ...
  it("should merge registry:theme with existing theme", async () => {
    ...
    await npxShadcn(fixturePath, ["add", "@one/foo-theme", "@two/theme", "--yes"]);
    expect(
      cssHasProperties(await fs.readFile(path.join(fixturePath, "app/globals.css"), "utf-8"), [
        {
          selector: ":root",
          properties: {
            "--background": "red",
    ...
    await npxShadcn(fixturePath, ["add", "@two/theme", "@one/foo-theme", "--yes"]);
    expect(
      cssHasProperties(await fs.readFile(path.join(fixturePath, "app/globals.css"), "utf-8"), [
        {
          selector: ":root",
  ...
  it("should add namespaced items from the shadcn registry", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init");
    await npxShadcn(fixturePath, ["add", "@shadcn/example-button", "@shadcn/no-framework-item"]);
    expect(await fs.pathExists(path.join(fixturePath, "components/example-button.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "path/to/foo.txt"))).toBe(true);
    expect(await fs.readFile(path.join(fixturePath, "path/to/foo.txt"), "utf-8")).toBe("Foo Bar");
  });
...
describe("registries:init", () => {
  ...
  it("should init from registry:style", async () => {
    ...
    await npxShadcn(fixturePath, ["init", "--force", "--no-reinstall", "@one/style"]);
  ...
  it("should init from registry:style with auth", async () => {
    ...
    await npxShadcn(fixturePath, ["init", "--force", "--no-reinstall", "@two/style"]);
