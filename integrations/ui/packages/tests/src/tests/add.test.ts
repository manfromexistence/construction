...
describe("shadcn add", () => {
  ...
  it("should add multiple items to project", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/card.tsx"))).toBe(true);
  });
  ...
  it("should add item from url", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "app/login/page.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/card.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/input.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/label.tsx"))).toBe(true);
  });

  it("should add component from local file", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app");
    await npxShadcn(fixturePath, ["init", "--defaults"]);
    await npxShadcn(fixturePath, ["add", "../../fixtures/registry/example-component.json"]);
  ...
  it("should add item with npm dependencies", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app");
    await npxShadcn(fixturePath, ["init", "--defaults"]);
    await npxShadcn(fixturePath, ["add", "../../fixtures/registry/example-style.json", "--yes"]);
    const packageJson = await fs.readJson(path.join(fixturePath, "package.json"));
  ...
  it("should add item with target", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app");
    await npxShadcn(fixturePath, ["init", "--defaults"]);
    await npxShadcn(fixturePath, ["add", "../../fixtures/registry/example-item.json"]);
  ...
  it("should add item with target to src", async () => {
    ...
    await npxShadcn(fixturePath, ["init", "--defaults"]);
    await npxShadcn(fixturePath, ["add", "../../fixtures/registry/example-item.json"]);
    expect(await fs.pathExists(path.join(fixturePath, "src/path/to/foo.txt"))).toBe(true);
    expect(await fs.readFile(path.join(fixturePath, "src/path/to/foo.txt"), "utf-8")).toBe(
      "Foo Bar"
    );
  });
  ...
  it("should add item with target to root", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app");
    await npxShadcn(fixturePath, ["init", "--defaults"]);
    await npxShadcn(fixturePath, ["add", "../../fixtures/registry/example-item-to-root.json"]);
    expect(await fs.pathExists(path.join(fixturePath, "config.json"))).toBe(true);
  ...
  it("should add item with envVars", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app");
    await npxShadcn(fixturePath, ["init", "--defaults"]);
    await npxShadcn(fixturePath, ["add", "../../fixtures/registry/example-env-vars.json"]);
    expect(await fs.pathExists(path.join(fixturePath, ".env.local"))).toBe(true);
    expect(await fs.readFile(path.join(fixturePath, ".env.local"), "utf-8")).toMatchInlineSnapshot(`
  ...
  it("should add NOT update existing envVars", async () => {
    ...
    await fs.writeFile(path.join(fixturePath, ".env.local"), "APP_URL=https://foo.com");
  ...
  it("should use existing .env if it exists", async () => {
    ...
    await fs.writeFile(path.join(fixturePath, ".env"), "APP_URL=https://foo.com");

    await npxShadcn(fixturePath, ["add", "../../fixtures/registry/example-env-vars.json"]);

    expect(await fs.pathExists(path.join(fixturePath, ".env.local"))).toBe(false);
    expect(await fs.readFile(path.join(fixturePath, ".env"), "utf-8")).toMatchInlineSnapshot(`
      "APP_URL=https://foo.com
  ...
  it("should add component to custom file path", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "custom/my-card.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/card.tsx"))).toBe(false);
  });
  ...
  it("should add component to custom directory", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "custom/components/card.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/card.tsx"))).toBe(false);
  });
  ...
  it("should add multiple files to custom directory", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "custom/ui/input.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "custom/ui/card.tsx"))).toBe(true);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/input.tsx"))).toBe(false);
    expect(await fs.pathExists(path.join(fixturePath, "components/ui/card.tsx"))).toBe(false);
  });
...
describe("shadcn registry add", () => {
  ...
  it("should add custom registry with URL", async () => {
    ...
    const componentsJson = await fs.readJson(path.join(fixturePath, "components.json"));
    expect(componentsJson.registries["@mycompany"]).toBe("https://example.com/r/{name}.json");
  });
  ...
  it("should error for registry not in index without URL", async () => {
    ...
    const result = await npxShadcn(fixturePath, ["registry", "add", "@nonexistent-registry-12345"]);
