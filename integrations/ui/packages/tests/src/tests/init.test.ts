...
describe("shadcn init - vite-app", () => {
  it("should init with custom alias and src", async () => {
    ...
    expect(await fs.pathExists(path.join(fixturePath, "src/components/ui/alert-dialog.tsx"))).toBe(
      true
    );

    expect(await fs.pathExists(path.join(fixturePath, "src/components/ui/button.tsx"))).toBe(true);

    const alertDialogContent = await fs.readFile(
      path.join(fixturePath, "src/components/ui/alert-dialog.tsx"),
      "utf-8"
    );
    expect(alertDialogContent).toContain('import { Button } from "#custom/components/ui/button"');
    expect(alertDialogContent).toContain('import { cn } from "#custom/lib/utils"');
  });
});
...
describe("shadcn init - custom style", async () => {
  ...
  it("should init with style that extends shadcn", async () => {
    ...
    expect(await fs.readFile(path.join(fixturePath, "lib/foo.ts"), "utf-8")).toBe(
      "const foo = 'bar'"
    );
  ...
  it("should init with style that extends another style", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app");
    await npxShadcn(fixturePath, ["init", "http://localhost:4445/r/style-extended.json"]);
  ...
  it("should init with custom style extended none", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app");
    await npxShadcn(fixturePath, ["init", "http://localhost:4445/r/style-extend-none.json"]);
    ...
    expect(await fs.pathExists(path.join(fixturePath, "lib/utils.ts"))).toBe(false);
...
describe("shadcn init - --name flag", () => {
  ...
  it("should create a new project with the specified name", async () => {
    ...
    expect(await fs.pathExists(path.join(projectPath, "components/theme-provider.tsx"))).toBe(true);
  });
  ...
  it("should create a new project with --name and -t vite", async () => {
    ...
    await npxShadcn(emptyDir, ["init", "--defaults", "--name", projectName, "-t", "vite"], {
      timeout: 120000,
    });
...
describe("shadcn init - next-monorepo", () => {
  ...
  itIfNotCi(
    ...
    async () => {
      ...
      expect(await fs.pathExists(projectPath)).toBe(true);
      expect(await fs.pathExists(path.join(projectPath, "packages/ui/components.json"))).toBe(true);
      expect(await fs.pathExists(path.join(projectPath, "apps/web/components.json"))).toBe(true);

      // Verify packages/ui/components.json is updated with preset config.
      const uiConfig = await fs.readJson(path.join(projectPath, "packages/ui/components.json"));
      ...
      const webConfig = await fs.readJson(path.join(projectPath, "apps/web/components.json"));
  ...
  itIfNotCi(
    ...
    async () => {
      ...
      const result = await npxShadcn(
        testBaseDir,
        ["init", "--name", projectName, "-t", "next", "--monorepo", "--preset", initUrl],
        { timeout: 300000 }
      );
      expect(result.exitCode).toBe(0);
...
describe("shadcn init - deprecated --src-dir", () => {
  it("should reject --src-dir as unknown option", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app");
    const result = await npxShadcn(fixturePath, ["init", "--defaults", "--src-dir"]);
...
describe("shadcn init - existing components.json", () => {
  ...
  it("should restore backup components.json on error", async () => {
    ...
    await npxShadcn(fixturePath, ["init", "invalid-component-that-does-not-exist"]);
