...
import { type RegistryItem, registrySchema } from "shadcn/schema";
...
const CACHE_ROOT = path.join(process.cwd(), "node_modules/.cache/build-registry");
const TRANSFORM_CACHE_ROOT = path.join(CACHE_ROOT, "transforms");
const TRANSFORM_CACHE_MANIFEST_PATH = path.join(CACHE_ROOT, "transform-manifest.json");
...
async function saveTransformCache() {
  ...
  const payload = Object.fromEntries(
    Array.from(transformCacheManifest.entries()).sort(([a], [b]) => a.localeCompare(b))
  );

  await fs.writeFile(TRANSFORM_CACHE_MANIFEST_PATH, JSON.stringify(payload, null, 2));
...
try {
  ...
  console.log("\n💅 Building styles...");
  await runWithConcurrency(stylesToBuild, CLI_BUILD_CONCURRENCY, async (style) => {
...
export const Index: Record<string, Record<string, any>> = {`;
  ...
  await writeIfChanged(outputPath, await formatGeneratedSource(index, outputPath));
}
...
async function buildBases(bases: Base[]) {
  const [baseImports, styleMaps] = await Promise.all([
    Promise.all(
      bases.map(async (base) => {
        ...
        const registryItems = result.data.items.filter((item) => item.type !== "registry:internal");

        const sourceFilePaths = Array.from(
          new Set(
            registryItems.flatMap((item) => normalizeRegistryFiles(item).map((file) => file.path))
          )
        );
            ...
            sourceFilePaths.map(
              async (filePath): Promise<readonly [string, string]> =>
                [
                  filePath,
                  await fs.readFile(
                    path.join(process.cwd(), `registry/bases/${base.name}/${filePath}`),
                    "utf8"
                  ),
                ] as const
            )
  ...
  for (const { base, baseRegistry, registryItems, sourceFiles } of baseImports) {
  ...
  await runWithConcurrency(
    ...
    async ({ base, style, baseRegistry, registryItems, sourceFiles, styleHash, styleMap }) => {
      ...
      await runWithConcurrency(filesToBuild, FILE_BUILD_CONCURRENCY, async (file) => {
        const source = sourceFiles.get(file.path);
        if (typeof source !== "string") {
          throw new Error(`Missing cached source for ${base.name}/${file.path}`);
        }

        const fileExtension = path.extname(file.path);
        const shouldTransform = fileExtension === ".tsx" || fileExtension === ".ts";
...
async function buildRegistryJsonFile(styleName: string) {
  ...
  const fixedRegistryJson = await formatGeneratedJson(fixedRegistry, registryJsonPath);
...
async function buildBlocksIndex() {
  ...
  await writeIfChanged(blocksJsonPath, await formatGeneratedJson(payload, blocksJsonPath));
}
...
async function buildConfig() {
  ...
  await writeIfChanged(outputPath, await formatGeneratedJson(config, outputPath));
}
...
async function copyUIToStyles() {
  await runWithConcurrency(STYLE_COMBINATIONS, COPY_CONCURRENCY, async ({ name: styleName }) => {
      ...
      await syncDirectory({
        ...
        transformContent: async (content, filePath, targetPath) => {
          ...
          return formatGeneratedSource(
            rewriteStyleDirectionImports(await transformDirection(content, true), styleName),
            targetPath
          );
        },
      });
...
async function buildIndex() {
  const baseUiRegistries = await Promise.all(
    Array.from(BASES).map(async (base) => {
      const { ui } = await import(`../registry/bases/${base.name}/ui/_registry.ts`);
    ...
    for (const item of items) {
      if (!componentMap.has(item.name)) {
        ...
        componentMap.set(item.name, {
          ...rest,
          ...(meta?.links ? { meta: { links: { [baseName]: meta.links } } } : {}),
        });
  ...
  const index = Array.from(componentMap.values()).sort((a, b) => a.name.localeCompare(b.name));
...
async function buildRegistriesJson() {
  ...
  await writeIfChanged(outputPath, await formatGeneratedJson(registries, outputPath));
}
...
function rewriteStyleDirectionImports(content: string, styleName: string) {
  return content.replaceAll(`@/styles/${styleName}/ui/`, `@/styles/${styleName}/ui-rtl/`);
}
...
async function buildColors() {
  ...
  await Promise.all(
    BASE_COLORS.map(async (baseColor) => {
      ...
      const cssVarKeys = Object.keys(light).filter((key) => !key.startsWith("sidebar"));

      const rootVars = cssVarKeys.map((key) => `    --${key}: ${light[key]};`).join("\n");
      ...
      await writeIfChanged(outputPath, await formatGeneratedJson(payload, outputPath));
