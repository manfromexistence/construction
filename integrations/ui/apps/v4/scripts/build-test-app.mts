...
const testAppsPath = args[1] || process.env.TEST_APPS_PATH || DEFAULT_TEST_APPS_PATH;
...
try {
  ...
  for (const { base, registryItems, styleMap } of builtRegistries) {
    ...
    const examplesDir = path.join(process.cwd(), `registry/bases/${base.name}/examples`);
    let exampleFiles: string[] = [];
    try {
      exampleFiles = (await fs.readdir(examplesDir)).filter(
        (f) => f.endsWith(".tsx") && !f.startsWith("_") && f !== "component-example.tsx" // Skip the generic component example.
      );
    } catch {
    ...
    for (const exampleFile of exampleFiles) {
      ...
      const exampleName = exampleFile.replace("-example.tsx", "").replace(".tsx", "");
    ...
    const blocksDir = path.join(process.cwd(), `registry/bases/${base.name}/blocks`);
...
async function buildBasesWithStyle(bases: Base[], style: (typeof STYLES)[0]) {
  ...
  for (const base of bases) {
    const { registry: baseRegistry } = await import(`../registry/bases/${base.name}/registry.ts`);
...
function rewriteImports(content: string, baseName: string) {
  ...
  content = content.replace(new RegExp(`@/registry/bases/${baseName}/lib/`, "g"), "@/lib/");
  content = content.replace(new RegExp(`@/registry/bases/${baseName}/hooks/`, "g"), "@/hooks/");
  content = content.replace(
...
async function updateLayoutStyle(testAppDir: string, styleName: string) {
  ...
  try {
    ...
    content = content.replace(/const STYLE = ["'][^"']*["']/, `const STYLE = "${styleName}"`);
