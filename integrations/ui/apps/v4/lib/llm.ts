...
function getComponentsList() {
  ...
  return list.map((component) => `- [${component.name}](${component.url})`).join("\n");
}
  ...
  return content.replace(componentPreviewRegex, (match, name) => {
    try {
      ...
      for (const base of BASES) {
        ...
        source = source.replaceAll(`@/examples/${base.name}/ui-rtl/`, "@/components/ui/");
        source = source.replaceAll(`@/examples/${base.name}/ui/`, "@/components/ui/");
