...
export function ChartDisplay({
  ...
  return (
    ...
    >
      <ChartToolbar chart={chart} className="relative z-20 flex justify-end px-3 py-2.5" />
      <div className="relative z-10 overflow-hidden rounded-xl bg-background">
        <ChartIframe src={`/view/${style}/${chart.name}`} height={460} title={chart.name} />
      </div>
    </div>
  );
}
...
export const getCachedRegistryItem = React.cache(async (name: string, styleName: Style["name"]) => {
