...
function isMainModule() {
  return process.argv[1] ? path.resolve(process.argv[1]) === scriptPath : false;
}
...
function getObjectPropertyAssignment(node: Node, propertyName: string): Node | undefined {
...
function inferRegistryDependenciesFromExampleName(exampleName: string, registryUiNames: string[]) {
...
function getRegistryImportBasePath(fromFilePath: string, moduleSpecifier: string) {
...
async function resolveRegistryImportPath(fromFilePath: string, moduleSpecifier: string) {
...
async function collectRegistryDependenciesFromFile(
  ...
  const sourceFile =
    project.getSourceFile(absolutePath) ?? project.addSourceFileAtPathIfExists(absolutePath);
  ...
  for (const moduleSpecifier of collectModuleSpecifiersFromSourceFile(sourceFile)) {
    ...
    const resolvedImport = await resolveRegistryImportPath(absolutePath, moduleSpecifier);
    ...
    if (typeof resolvedImport !== "string") {
      missingFiles.push(`${resolvedImport.sourceFile} -> ${resolvedImport.missingImport}`);
      continue;
    }
...
async function collectExampleRegistryDependencies(project: Project, filePaths: string[]) {
...
function setRegistryDependencies(node: Node, dependencies: string[]) {
  ...
  const existingProperty = getObjectPropertyAssignment(node, "registryDependencies");
  const filesPropertyIndex = node.getProperties().findIndex((property) => {
  ...
  const insertIndex = filesPropertyIndex >= 0 ? filesPropertyIndex : node.getProperties().length;
...
function formatIssue(issue: ExampleIssue) {
  const fileList = issue.exampleFiles.length > 0 ? issue.exampleFiles.join(", ") : "no files";
  const extraDependencies =
    issue.extraDependencies.length > 0 ? issue.extraDependencies.join(", ") : "none";
  const missingDependencies =
    issue.missingDependencies.length > 0 ? issue.missingDependencies.join(", ") : "none";
  const missingFiles = issue.missingFiles.length > 0 ? issue.missingFiles.join(", ") : "none";
...
export async function syncExampleRegistryDependencies({
  ...
  const registryExamplesSourceFile = project.addSourceFileAtPath(registryExamplesPath);
  const registryUiSourceFile = project.addSourceFileAtPath(registryUiPath);

  const initializer = getRegistryItemsDeclaration(registryExamplesSourceFile, "examples");
  ...
  for (const item of initializer.getElements()) {
    ...
    const expectedDependencies = dedupe([...importedDependencies, ...inferredDependencies]).sort(
      (left, right) => left.localeCompare(right)
    );

    const normalizedCurrentDependencies = dedupe(currentDependencies);
    const currentMagicUiDependencies = normalizedCurrentDependencies.filter(isMagicUiPackage);
    ...
    const dependenciesChanged =
      ...
      nextDependencies.some((dependency, index) => dependency !== currentDependencies[index]);
