...
async function askCssPath() {
  const result = await p.text({
    ...
    validate(value) {
      if (!value.trim()) return "Path is required. Please enter a valid one.";
    },
  });
...
async function askComponentsAlias() {
  const result = await p.text({
    ...
    validate(value) {
      if (!value.trim()) return "Alias is required. Please enter a valid one.";
    },
  });
...
async function askUtilsAlias() {
  const result = await p.text({
    ...
    validate(value) {
      if (!value.trim()) return "Alias is required. Please provide a valid value.";
    },
  });
