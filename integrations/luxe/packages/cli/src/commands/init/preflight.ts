...
export async function preFlight() {
  ...
  const { dependencies, devDependencies } = JSON.parse(packageJsonFileContent) as {
    dependencies: Record<string, string>;
    devDependencies: Record<string, string>;
  };

  const isTailwindInDeps = TAILWIND_PACKAGE in dependencies || TAILWIND_PACKAGE in devDependencies;

  if (!isTailwindInDeps) {
    errorsFound[InitCommandErrors.TAILWIND_NOT_INSTALLED] = true;
  } else {
    const version = dependencies[TAILWIND_PACKAGE] || devDependencies[TAILWIND_PACKAGE];
