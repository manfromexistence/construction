...
export async function fetchComponentRegistry(componentName: string) {
  try {
    const response = await fetch(`${apiConfig.luxeRegistry}/${componentName}.json`, {
      method: "GET",
      headers: {
    ...
    if (!parsed.success) {
      logger.warning(`Component ${componentName} failed schema validation and will be skipped.`);
      return null;
    }
...
export async function writeComponentFileFromTemplate(componentRegistryEntry: Registry, eta: Eta) {
  ...
  try {
    ...
    const componentsDirPath = resolveAliasToAbsolutePath(getManifest.aliases.components);
...
export function logComponentSummary(installedComponents: string[]) {
  ...
  const componentsPath = resolveAliasToAbsolutePath(getManifest.aliases.components);

  const formattedList = installedComponents
    .map((component) => `  ${chalk.gray("•")} ${chalk.green(pascalCase(component))}`)
    .join("\n");
