...
export async function handler(availableComponents: string[], cliArgs: string[]) {
  ...
  if (existsSync(componentsDirPath)) {
    ...
    const existingComponentNames = existingFiles.map((file) => path.parse(file).name);
  ...
  if (skippedComponents.length > 0) {
    logger.info(`${chalk.cyan(skippedComponents.join(", "))} already existed and were kept.`);
