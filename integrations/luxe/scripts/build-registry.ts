...
const IGNORED_EXTERNAL_DEPENDENCIES = new Set(["react", "motion/react", "lucide-react"]);
const COMPONENTS_PATH = path.resolve(process.cwd(), "packages/react/src/components");
...
async function main() {
  try {
    ...
    for (const componentPath of componentPaths) {
      ...
      const { externalDependencies, internalDependencies } = getDependencies(importPaths);
      ...
      await saveRegistryFile("components", name, JSON.stringify(registryEntry, null, 2));
...
async function saveRegistryFile(type: RegistryType, filename: string, content: string) {
  const fileJSON = `${filename}.json`;

  try {
    await fs.writeFile(`${WWW_PUBLIC_PATH}/registry/${type}/${fileJSON}`, content, "utf8");
